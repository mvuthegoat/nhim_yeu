...
document.addEventListener('DOMContentLoaded', (event) => {
    const hoaButton = document.getElementById('hoa-button');
    const hoaAudio = document.getElementById('hoa-audio');

    hoaButton.addEventListener('click', (e) => {
        e.preventDefault();  // Prevent the default action of the link
        hoaAudio.play();     // Play the audio

        // Redirect to the link after the audio has finished playing
        hoaAudio.onended = () => {
            window.location.href = hoaButton.href;
        };
    });
});
